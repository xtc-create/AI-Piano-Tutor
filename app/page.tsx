    ...
    <main style={{ minHeight: '100vh', background: '#161616', color: '#f0ece5', fontFamily: 'Inter, ui-sans-serif, system-ui, sans-serif', padding: '42px 20px' }}>
      <div style={{ maxWidth: 1000, margin: '0 auto' }}>
        ...
        <section style={panelStyle}>
          ...
          <div style={{ display: 'flex', gap: 7, flexWrap: 'wrap', marginTop: 14, fontFamily: 'Arial, sans-serif' }}>{Object.entries(KEYBOARD_MAP).map(([key, note]) => <span key={key} style={{ padding: '5px 8px', borderRadius: 5, background: '#302f2c', color: activeNotes.has(note) ? '#e0c28c' : '#d1cbc0', fontSize: 12 }}><b>{key.toUpperCase()}</b> {note}</span>)}</div>
        </section>
        <section style={{ ...panelStyle, marginTop: 18 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 15, flexWrap: 'wrap' }}><div><h2 style={{ margin: 0, fontSize: 18 }}>Recording</h2><p style={{ margin: '5px 0 0', color: '#aaa39a', fontSize: 13, fontFamily: 'Arial, sans-serif' }}>{recording ? 'Capturing note events with performance timing…' : 'Record a phrase, then replay its original timing.'}</p></div><button onClick={toggleRecording} style={{ ...buttonStyle, background: recording ? '#9f3d32' : '#6f5c3e' }}>{recording ? 'Stop recording' : 'Start recording'}</button></div>
        </section>
        <section style={{ ...panelStyle, marginTop: 18 }}><div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}><h2 style={{ margin: 0, fontSize: 18 }}>History</h2><button onClick={resetHistory} disabled={!sequences.length} style={{ ...buttonStyle, opacity: sequences.length ? 1 : .45 }}>Reset history</button></div>
...
const panelStyle = { background: '#252422', border: '1px solid #4b4842', borderRadius: 16, padding: 20, boxShadow: '0 18px 50px rgba(0,0,0,.18)' }
...
function AuthScreen({ registering, onNavigate, onAuthenticated, onTryGuest }: { registering: boolean; onNavigate: (path: string) => void; onAuthenticated: (account: Account) => void; onTryGuest: () => void }) {
  ...
  return <main style={{ minHeight: '100vh', display: 'grid', placeItems: 'center', padding: 20, color: '#f0ece5', background: 'radial-gradient(circle at 20% 10%, #333333 0, transparent 32%), #151515', fontFamily: 'Inter, ui-sans-serif, system-ui, sans-serif' }}>
    <form onSubmit={submit} style={{ width: 'min(100%, 430px)', background: 'rgba(35,35,34,.96)', border: '1px solid #55514a', padding: 30, borderRadius: 20, boxShadow: '0 28px 80px #0008' }}>
      <p style={{ margin: 0, color: '#c5a36a', fontWeight: 700, fontSize: 12, letterSpacing: '.14em', fontFamily: 'Arial, sans-serif' }}>PIANO TUTOR</p><h1 style={{ margin: '10px 0 7px', fontSize: 32 }}>{registering ? 'Build your practice space' : 'Welcome back'}</h1><p style={{ color: '#b9b2a8', lineHeight: 1.5, marginBottom: 24, fontFamily: 'Arial, sans-serif' }}>{registering ? 'Save progress, goals, and practice history to your account on this device.' : 'Sign in to continue your personal learning path.'}</p>
